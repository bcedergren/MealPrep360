...
// POST /api/meal-plans/[id]/days/[dayIndex]/unskip - Unskip meal plan day
export async function POST(
	...
	try {
		...
		console.log(
			...
			`${API_CONFIG.endpoints.mealPlans}/${id}/days/${dayIndex}/unskip`,
			'Body:',
			body
		);
		...
		if (!response.success) {
			...
			try {
				...
				// Update the specific day to planned
				...
				// Mark the day as planned (restore original recipe if available)
				...
				return NextResponse.json({
					success: true,
					message: 'Meal plan day unskipped successfully',
